...
std::unique_ptr<Annotation> prover::MakeDerefAssignPost(PostInfo info, const cola::Dereference& lhs, const cola::Expression& rhs) {
...
inline std::array<Symbol, 5> MakeQVars(std::shared_ptr<Encoder> encoder, const PostConfig& config) {
	...
	return prover::MakeQuantifiedVariables(encoder, nodeType, nodeType, valueType, valueType, valueType);
}
