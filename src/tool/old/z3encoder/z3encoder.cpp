...
std::vector<std::pair<std::string, const Type*>> GetPointerFields(const SolverConfig& config) { // TODO: needed?
	...
	for (auto [fieldName, fieldType] : config.flowDomain->GetNodeType().fields) {
		...
		result.emplace_back(fieldName, &fieldType.get());
	}
	return result;
}
...
Z3Encoder::Z3Encoder(std::shared_ptr<SolverConfig> config) :
    Encoder(config),
