...
namespace heal {
	...
	struct PropertyInstantiationError : public std::exception {
		...
		PropertyInstantiationError(std::string name, std::size_t index, const cola::Type& expected, const cola::Type& got)
			: cause("Instantiation of property '" + name + "' failed: expected argument at postion " + std::to_string(index+1) +
		                " to be of type '" + expected.name + "' but got incompatible type '" + got.name + "'." ) {}
		virtual const char* what() const noexcept { return cause.c_str(); }
	};


	template<std::size_t N = 1, typename T = ConjunctionFormula, EXTENDS_FORMULA(T)>
	struct Property {
		std::string name;
		...
		std::unique_ptr<T> instantiate(std::array<std::reference_wrapper<const cola::VariableDeclaration>, N> decls) const {
			...
			return heal::replace_expression(heal::copy(*blueprint), [&dummy2real](const cola::Expression& expr){
				auto [is_var, var_expr] = heal::is_of_type<cola::VariableExpression>(expr);
				if (is_var) {
