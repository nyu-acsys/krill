...
bool heal::syntactically_equal(const Expression& expression, const Expression& other) {
...
struct LogicSyntacticEqualityChecker : public LogicVisitor {
	...
	void visit(const DataStructureLogicallyContainsAxiom& formula) override {
		handle(formula, [](const auto& formula, const auto& other){
			return heal::syntactically_equal(*formula.value, *other.value);
		});
	}

	void visit(const NodeLogicallyContainsAxiom& formula) override {
		handle(formula, [](const auto& formula, const auto& other){
			return heal::syntactically_equal(*formula.node, *other.node) && heal::syntactically_equal(*formula.value, *other.value);
		});
	}
	...
	void visit(const ObligationAxiom& formula) override {
		handle(formula, [](const auto& formula, const auto& other){
			return formula.kind == other.kind && heal::syntactically_equal(*formula.key, *other.key);
		});
	}

	void visit(const FulfillmentAxiom& formula) override {
		handle(formula, [](const auto& formula, const auto& other){
			return formula.kind == other.kind && formula.return_value == other.return_value && heal::syntactically_equal(*formula.key, *other.key);
		});
	}
	...
	void visit(const FuturePredicate& formula) override {
		handle(formula, [this](const auto& formula, const auto& other){
			return is_equal(*formula.pre, *other.pre) && is_equal(*formula.post, *other.post)
			    && heal::syntactically_equal(*formula.command->lhs, *other.command->lhs)
			    && heal::syntactically_equal(*formula.command->rhs, *other.command->rhs);
		});
	}
...
bool heal::syntactically_equal(const Formula& formula, const Formula& other) {
