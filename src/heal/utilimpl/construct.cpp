...
std::unique_ptr<Dereference> heal::MakeDerefExpr(const VariableDeclaration& decl, std::string field) {
	return std::make_unique<Dereference>(MakeExpr(decl), field);
}
...
std::unique_ptr<NegatedAxiom> heal::MakeNegatedAxiom(std::unique_ptr<Axiom> axiom) {
...
std::unique_ptr<ConjunctionFormula> heal::MakeConjunction(std::deque<std::unique_ptr<SimpleFormula>> conjuncts) {
	auto result = std::make_unique<ConjunctionFormula>();
	result->conjuncts = std::move(conjuncts);
	return result;
}

std::unique_ptr<AxiomConjunctionFormula> heal::MakeAxiomConjunction() {
	return std::make_unique<AxiomConjunctionFormula>();
}

std::unique_ptr<AxiomConjunctionFormula> heal::MakeAxiomConjunction(std::deque<std::unique_ptr<Axiom>> conjuncts) {
...
std::unique_ptr<ImplicationFormula> heal::MakeImplication(std::unique_ptr<Axiom> premise, std::unique_ptr<Axiom> conclusion) {
	auto result = std::make_unique<ImplicationFormula>();
	result->premise->conjuncts.push_back(std::move(premise));
	result->premise->conjuncts.push_back(std::move(conclusion));
	return result;
}

std::unique_ptr<ImplicationFormula> heal::MakeImplication(std::unique_ptr<AxiomConjunctionFormula> premise, std::unique_ptr<AxiomConjunctionFormula> conclusion) {
	auto result = std::make_unique<ImplicationFormula>();
