#pragma once
#ifndef PROVER_ENCODER
#define PROVER_ENCODER
...
namespace prover {
	...
	struct Encoder {
		...
		virtual Term EncodeUniqueInflow(Term node, EncodingTag tag) = 0;
		...
		inline Term EncodeNowUniqueInflow(Term node) { return EncodeUniqueInflow(node, EncodingTag::NOW); }
		...
		inline Term EncodeNextUniqueInflow(Term node) { return EncodeUniqueInflow(node, EncodingTag::NEXT); }
