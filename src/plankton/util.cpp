...
QuantifiedVariableGenerator::QuantifiedVariableGenerator(std::shared_ptr<Encoder> encoder_) : encoder(std::move(encoder_)) {}

Symbol QuantifiedVariableGenerator::GetNext(const cola::Type& type) {
	auto& decl = GetDeclaration(counter++, type);
	return encoder->Encode(decl, EncodingTag::NOW);
}


std::vector<Symbol> plankton::MakeQuantifiedVariables(std::shared_ptr<Encoder> encoder, std::vector<std::reference_wrapper<const cola::Type>> types) {
