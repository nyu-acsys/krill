...
bool remove(data_t key) {
	...
	while (true) {
		...
		if (k > key) {
            ...
            next = curr->next;
 			// if (CAS(<pred->marked, pred->next, curr->marked, curr->next>, <false, curr, false, next>, <false, next, true, next>)) { // correct
			if (CAS(<pred->marked, pred->next, curr->marked, curr->next>, <false, curr, false, next>, <false, next, false, next>)) { // buggy: no logical deletion
				return true;
			}
		}
	}
